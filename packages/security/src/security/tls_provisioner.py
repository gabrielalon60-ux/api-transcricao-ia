#!/usr/bin/env python3
"""Authoritative PostgreSQL TLS material provisioning into named volume from base64 secrets."""
...
def decode_b64_secret(var_name: str, raw_value: str | None) -> bytes:
    """Safely decodes a base64 environment variable without leaking contents."""
...
def validate_and_provision_tls(
    ...
    server_key_uid: int = 70,
    server_key_gid: int = 70,
    server_cert_uid: int = 0,
    server_cert_gid: int = 0,
    ca_cert_uid: int = 0,
    ca_cert_gid: int = 0,
    overwrite: bool = False,
) -> dict[str, Path]:
    """Validates cryptographic integrity in-memory first, then provisions TLS files with strict POSIX permissions."""
    ...
    # --- Phase 1: In-memory Cryptographic Validation (Fail-closed before writing) ---
    ...
    # 3. Validate Server Certificate SAN contains DNS:platform-db
    try:
    ...
    cert_pub = server_cert.public_key()
    if isinstance(server_key, rsa.RSAPrivateKey) and isinstance(cert_pub, rsa.RSAPublicKey):
        if server_key.public_key().public_numbers() != cert_pub.public_numbers():
            raise ValueError("Server private key does NOT match server certificate public key")

    # --- Phase 2: Safe File Materialization with Atomic / Direct Replace ---
    ...
    if os.name != "nt":
        ...
        if hasattr(os, "chown") and (ca_cert_uid != 0 or ca_cert_gid != 0):
            os.chown(ca_crt_path, ca_cert_uid, ca_cert_gid)  # type: ignore[attr-defined]

    # Write Server Certificate
    if server_crt_path.exists():
        server_crt_path.unlink()
    ...
    if os.name != "nt":
        ...
        if hasattr(os, "chown") and (server_cert_uid != 0 or server_cert_gid != 0):
            os.chown(server_crt_path, server_cert_uid, server_cert_gid)  # type: ignore[attr-defined]

    # Write Server Private Key (0600 and chown to PostgreSQL server UID/GID)
    ...
    if os.name != "nt":
        ...
        if hasattr(os, "chown"):
            os.chown(server_key_path, server_key_uid, server_key_gid)  # type: ignore[attr-defined]
...
def main(argv: list[str] | None = None) -> int:
    """CLI entrypoint for PostgreSQL TLS provisioning."""
    ...
    parser.add_argument(
        "--server-key-uid",
        type=int,
        default=int(os.environ.get("POSTGRES_TLS_KEY_UID", "70")),
        help="Target UID owner for server private key (default: 70 for postgres:15-alpine)",
    )
    parser.add_argument(
        "--server-key-gid",
        type=int,
        default=int(os.environ.get("POSTGRES_TLS_KEY_GID", "70")),
        help="Target GID owner for server private key (default: 70 for postgres:15-alpine)",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    try:
        ...
        results = validate_and_provision_tls(
            ...
            server_key_uid=args.server_key_uid,
            server_key_gid=args.server_key_gid,
        ...
        print(f"  Server Private Key:  {results['server_key']} (Mode 0600, Owner UID:{args.server_key_uid}, GID:{args.server_key_gid})")
