...
from transcription.database.models import Request, RequestStatus, Application
from transcription.schemas.requests import RequestOut, RequestListResponse
...
def list_requests(
    application_id: Optional[uuid.UUID] = Query(
        None, description="Filter by application UUID."
    ),
    status: Optional[RequestStatus] = Query(None, description="Filter by status."),
    date_from: Optional[datetime] = Query(
        None, description="Filter requests created after this date (ISO 8601)."
    ),
    date_to: Optional[datetime] = Query(
        None, description="Filter requests created before this date (ISO 8601)."
    ),
    ...
    return RequestListResponse(
        total=total, items=[RequestOut.model_validate(req) for req in items]
    )
